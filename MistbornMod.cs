...
namespace MistbornMod
{
    public class MistbornMod : Mod
    {
        ...
        public void SaveConfig(ModConfig config)
        {
            if (config != null)
            {
                // Use reflection to access the private Save method in ModConfig
                var saveMethod = typeof(ModConfig).GetMethod(
                    "Save",
                ...
                {
                    // Alternative method if reflection fails
                    try
                    {
                        // This will trigger ModConfig's OnChanged which should save
                        var field = config.GetType().GetField("UIScale");
                        if (field != null)
                        {
                            float currentValue = (float)field.GetValue(config);
                            field.SetValue(config, currentValue + 0.0001f); // Tiny change that won't be visible
                            field.SetValue(config, currentValue); // Reset back
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public static void CreateZincStalagmite(int centerX, int bottomY, int height, int baseWidth)
        {
            ...
            for (int y = 0; y < height + embedDepth; y++) // Add embedDepth to total height
            {
                ...
                if (y < embedDepth)
                    heightRatio = 1.0f; // Full width while underground
                ...
                for (int w = -currentWidth / 2; w <= currentWidth / 2; w++)
                {
                    ...
                    if (
                        currentX < 5
                        || currentX >= Terraria.Main.maxTilesX - 5
                        || currentY < 5
                        || currentY >= Terraria.Main.maxTilesY - 5
                    )
                        continue;
                    ...
                    if (tile.HasTile)
                    {
                        ...
                        if (
                            tile.TileType == Terraria.ID.TileID.Hellstone
            ...
            for (int i = 0; i < baseWidth * 2; i++)
            {
                ...
                if (
                    scatterX < 5
                    || scatterX >= Terraria.Main.maxTilesX - 5
                    || scatterY < 5
                    || scatterY >= Terraria.Main.maxTilesY - 5
                )
                    continue;
